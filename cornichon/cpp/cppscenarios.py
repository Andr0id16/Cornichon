...
class PrintScenario(common.PrintScenario):
    def __init__(self, settings):
        super().__init__()
        self.line = f"\n{settings['indent']*3}std::clog << %s << std::endl;"
        self.contractions = {' << ""': '', ' "" << ': ' '}
        self.sub = '" << %s << "'
        self.step = """
[[indent]][[indent]]/// Gherkin DSL step
[[indent]][[indent]]void [[stepName]]([[arguments]])[[braceIndentL2]]{
[[description]]
[[indent]][[indent]]}
...
"""[1:]
        ...
        self.step = self.step.replace("[[braceIndentL2]]", braceIndentL2)
...
def Generate(parsed, settings):
    ...
    printer = PrintScenario(settings)
...
"""[1:]
    ...
    concat = concat.replace("[[braceSep]]", settings['bracesep'])
...
def ReGenerate(parsed: tuple[list, str], settings: dict):
    ...
    # In case of no new scenarios and no extra spaces the 'concat =
    # concat[:-2] + """' line removes "{"
    concat = """
namespace [[fullnamespace]][[braceSep]]{
"""[1:]
